...
$query = "
	SELECT c.category_id, c.name, sc.category_id AS subcategory_id, sc.name AS subcategory_name
	FROM categories c
	LEFT JOIN categories sc ON c.category_id = sc.parent_id
...
if ($result->num_rows > 0) {
    // Initialize an array to store categories and subcategories
    $categories = array();

    // Loop through the result set and organize categories and subcategories
    while ($row = $result->fetch_assoc()) {
        // If the category has no subcategory yet, initialize an empty array
        ...
        // If there is a subcategory, add it to the corresponding category
        if (!is_null($row['subcategory_id'])) {
            $categories[$row['category_id']]['subcategories'][] = array(
    ...
    foreach ($categories as $category) {
        ...
        // If the category has subcategories, output them
        if (!empty($category['subcategories'])) {
            echo '<ul>';
            foreach ($category['subcategories'] as $subcategory) {
                echo '<li><a href="category.php?id=' . $subcategory['id'] . '">' . $subcategory['name'] . '</a></li>';
            }
            echo '</ul>';
        }
